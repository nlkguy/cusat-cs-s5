...
def snake():
    ...
    for i in range(1,snakeSegCount):
        colorA,colorB,colorC = random.randint(0, 10)/10,random.randint(0, 10)/10,random.randint(0, 10)/10
        print(f"snake segment colors ({colorA},{colorB},{colorC})")
        glColor3f(colorA,colorB,colorC) #magenta? violet? idk 
        glPointSize(snakeSize)
        glBegin(GL_POINTS)
        #print(f"snake at ({snakeX},{snakeY})")
